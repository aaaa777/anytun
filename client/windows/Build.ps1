...
if ($Ci) {
    iscc.exe /dMyAppVersion="$appVersion" /dMyAppInstallerName="AnytunInstaller" "/SMySignTool=.\signtool.exe sign /v /f `$qD:\a\anytun\anytun\GitHubActionsWorkflow.pfx`$q /p `"${{ secrets.Pfx_Passphrase }}`" /t http://timestamp.comodoca.com/authenticode /fd SHA256 `$p `$f" $(Join-Path $scriptDir ".\installer.iss")
} else {
